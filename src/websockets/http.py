...
async def read_request(stream: asyncio.StreamReader) -> Tuple[str, "Headers"]:
    ...
    :param ~asyncio.StreamReader stream: input to read the request from
...
async def read_response(stream: asyncio.StreamReader) -> Tuple[int, str, "Headers"]:
    """
    Reads an HTTP/1.1 response and returns ``(status_code, reason, headers)``.
    ...
    :param ~asyncio.StreamReader stream: input to read the response from
...
class Headers(MutableMapping[str, str]):
    ...
    def get_all(self, key: str) -> List[str]:
        """
        Returns the (possibly empty) list of all values for a header.
